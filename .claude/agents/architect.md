...
2. Sketch the change in 2-5 bullets before editing. Scale the thinking to the decision:
   - hard to reverse (data shape, file format, public API, dependency) → **widen-the-solution-space** then **weigh-tradeoffs** (`.claude/skills/<name>/SKILL.md`); record the winner as an ADR
   - touches I/O, user input, external services, concurrency, or persisted state → **premortem**
   - defines a boundary other code will depend on → **interface-contracts**
   - codes against a library, SDK, CLI flag, or version-specific behavior → **search-first**
