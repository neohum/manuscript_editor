...
import { analyzeDiff, type SloppinessReport } from "./sloppiness.ts";
...
export function writeCapsule({ card, from = "?", to = "next", health = null }: { card: string; from?: string; to?: string; health?: string | null }): Capsule {
  ...
  let sloppiness: SloppinessReport | undefined;
  try {
    sloppiness = analyzeDiff(ROOT, baseline);
  } catch {}
...
export function capsuleMarkdown(capsule?: Capsule | null): string {
  ...
  if (capsule.sloppiness && capsule.sloppiness.filesAnalyzed > 0) {
    lines.push(`Sloppiness metric: ${capsule.sloppiness.summary}`);
    if (capsule.sloppiness.highCcFunctions.length > 0) {
      const fnSummary = capsule.sloppiness.highCcFunctions.slice(0, 3).map((f) => `${f.name}() CC:${f.cc}`).join(", ");
      lines.push(`  High-complexity functions (CC > 10): ${fnSummary}${capsule.sloppiness.highCcFunctions.length > 3 ? " …" : ""}`);
    }
  }
